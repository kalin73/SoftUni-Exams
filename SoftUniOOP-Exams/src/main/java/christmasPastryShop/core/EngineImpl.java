...
public class EngineImpl implements Engine {
	...
	private String processInput() throws IOException {
		...
		switch (command) {
		case AddCocktail:
			result = this.controller.addCocktail(data[0], data[1], Integer.parseInt(data[2]), data[3]);
			break;
		case AddDelicacy:
			result = this.controller.addDelicacy(data[0], data[1], Double.parseDouble(data[2]));
			break;
		case AddBooth:
			result = this.controller.addBooth(data[0], Integer.parseInt(data[1]), Integer.parseInt(data[2]));
			break;
		case ReserveBooth:
